...
def draw_page(ax, room, school_name, update_year, rows, header_mode, header_image, school_name_header, directorate_name, academy_name):
    ...
    if header_mode == "custom" and header_image is not None:
        ax.imshow(header_image, extent=[0.05, 0.95, 0.855, 0.96], aspect="auto", zorder=3)
    elif header_mode == "fixed" and header_image is not None:
        ax.imshow(header_image, extent=[0.05, 0.95, 0.865, 0.96], aspect="auto", zorder=2)
        ax.text(
            0.88,
            0.905,
            rtl_text(school_name_header),
            ha="right",
            va="top",
            **text_kwargs(HEADER_FONT_PROP, size=11, weight="bold"),
        )
        ax.text(
            0.88,
            0.878,
        ...
        ax.text(
            ...
            ha="right",
            va="top",
            **text_kwargs(HEADER_FONT_PROP, size=10.5),
        )
        ax.text(
            0.62,
            0.885,
            ...
            ha="right",
            va="top",
            **text_kwargs(HEADER_FONT_PROP, size=8.7),
        )
        ax.text(
            0.62,
            0.865,
    ...
    ax.text(0.5, 0.812, rtl_text("بطاقة توطين المجرود"), ha="center", va="center", **text_kwargs(BODY_FONT_PROP, size=16, weight="bold"))

    draw_box(ax, 0.035, 0.738, 0.34, 0.035)
    ax.text(0.43, 0.755, rtl_text("المكان :"), ha="left", va="center", **text_kwargs(BODY_FONT_PROP, size=11.5, weight="bold"))
    ax.text(0.355, 0.755, rtl_text(room), ha="right", va="center", **text_kwargs(BODY_FONT_PROP, size=9.4))

    draw_box(ax, 0.035, 0.700, 0.34, 0.035)
    ax.text(0.43, 0.717, rtl_text("تاريخ التحيين :"), ha="left", va="center", **text_kwargs(BODY_FONT_PROP, size=11.5, weight="bold"))
    ax.text(0.355, 0.717, rtl_text(update_year), ha="right", va="center", **text_kwargs(BODY_FONT_PROP, size=9.4))

    ax.text(0.18, 0.165, rtl_text("توقيع رئيس المؤسسة"), ha="center", va="center", **text_kwargs(BODY_FONT_PROP, size=10))
    ax.text(0.72, 0.165, rtl_text("توقيع مسير المصالح المادية والمالية"), ha="center", va="center", **text_kwargs(BODY_FONT_PROP, size=10))
